import { bookingConfirmationHtml, contactAckHtml } from "./email-templates";

const RESEND_KEY = import.meta.env.RESEND_API_KEY ?? process.env.RESEND_API_KEY ?? "";
const FROM = import.meta.env.EMAIL_FROM ?? process.env.EMAIL_FROM ?? "Core & Balance <onboarding@resend.dev>";
...
export async function sendBookingConfirmation(args: {
  ...
  return sendEmail({
    to: args.to,
    subject: `Booking confirmed — ${args.className}, ${args.when}`,
