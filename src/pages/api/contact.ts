...
export const POST: APIRoute = async ({ request }) => {
  ...
  if (!name || !email || !message || message.length < 10) {
    return new Response(JSON.stringify({ error: "Please fill all fields (message at least 10 characters)" }), { status: 400 });
  }
  if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
    return new Response(JSON.stringify({ error: "Invalid email" }), { status: 400 });
  }

  // contact_messages has an anon INSERT policy — no service key needed
  const sb = anonClient();
  if (!sb) {
    return new Response(JSON.stringify({ error: "Server not configured" }), { status: 500 });
  }
  const { error } = await sb.from("contact_messages").insert({ name, email, message, source_ip: ip });
  if (error) {
    return new Response(JSON.stringify({ error: "Could not save your message — please try again" }), { status: 500 });
  }
